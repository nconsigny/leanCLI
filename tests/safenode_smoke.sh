...
#      from $KOHAKU_SAFE_NODE_API_KEY (or the literal new key inlined below
...
#   KOHAKU_SAFE_NODE_API_KEY  override the inlined user key
...
KEY="${KOHAKU_SAFE_NODE_API_KEY:-olabs-api-bf83325bdef58f70006bf6ee1245cb4bbd475b4d0083b257144cb6889240d35b}"
URL="https://rpc.safe-node.com/$KEY/json_rpc"
SOCK="${DAEMON_SOCKET:-/run/user/$(id -u)/leankohaku/leankohaku.sock}"
...
export KOHAKU_SAFE_NODE_URL="${KOHAKU_SAFE_NODE_URL:-https://rpc.safe-node.com}"
export KOHAKU_SAFE_NODE_API_KEY="$KEY"
export KOHAKU_SAFE_NODE_DOMAIN="${KOHAKU_SAFE_NODE_DOMAIN:-rpc.safe-node.com}"
...
./.lake/build/bin/leankohaku daemon stop 2>/dev/null || true
for _ in 1 2 3 4 5; do [ -S "$SOCK" ] || break; sleep 1; done
if [ -S "$SOCK" ]; then
  echo "daemon did not stop via RPC; killing"
  pkill -f leankohaku-daemon || true
  pkill -f 'bridge/(helios|colibri|safenode)/bridge.mjs' || true
  sleep 1
  rm -f /run/user/"$(id -u)"/leankohaku/*.sock
fi
nohup ./.lake/build/bin/leankohaku-daemon > /tmp/leankohaku-daemon.out 2>&1 &
echo "spawned leankohaku-daemon pid=$! (logs: /tmp/leankohaku-daemon.out)"
...
for i in $(seq 1 30); do
  if [ -S "$SOCK" ] && grep -q '\[safenode\] attested TLS pin' /tmp/leankohaku-daemon.out 2>/dev/null; then
    echo "safenode attested (after ${i}s)"
    break
  fi
  sleep 1
done
grep -E '\[safenode\]|safenode attested|helios enabled' /tmp/leankohaku-daemon.out || true
