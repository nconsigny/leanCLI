#!/usr/bin/env bash
# Phase 0 smoke test for the Lean-native kohaku-agent.
#
# Three stages, each opt-in via env flags:
#
#   Stage A — pure transport (always runs):
#     `kohaku-agent --rpc '{ping,version,bogus-method}'` exit + stdout shape.
#     No daemon, no llama-server needed.
#
#   Stage B — daemon round-trip (runs when LEANKOHAKU_SOCKET is set
#     and the socket exists):
#     Uses one tool RPC against a running daemon to confirm the
#     UDS JSON-RPC path is wired.
#
#   Stage C — full agent loop (only when KOHAKU_TEST_LLAMA_URL is set):
...
#     `KOHAKU_TEST_MOCK_LLM=1` instructs the script to skip stage C
...
if [[ ! -x "$AGENT" ]]; then
  echo "FAIL: $AGENT not built. Run \`lake build kohaku_agent\` first." >&2
  exit 1
fi
...
SOCKET="${LEANKOHAKU_SOCKET:-${XDG_RUNTIME_DIR:-/tmp}/leankohaku/leankohaku.sock}"
if [[ ! -S "$SOCKET" ]]; then
  note "skipped: no daemon socket at $SOCKET"
  note "  start the daemon with \`lake env .lake/build/bin/leankohaku-daemon\`"
  note "  then re-run with LEANKOHAKU_SOCKET=$SOCKET"
else
  # We do not have a Lean-side one-shot JSON-RPC CLI in this tree.
  # Use the existing kohaku-llm-legacy daemon-callback wire shape:
...
if [[ "${KOHAKU_TEST_MOCK_LLM:-0}" = "1" ]]; then
  note "skipped: KOHAKU_TEST_MOCK_LLM=1 set (sandbox mode)"
  note "  to run stage C, start llama-server on http://127.0.0.1:8080,"
  note "  set KOHAKU_TEST_LLAMA_URL=http://127.0.0.1:8080/v1/chat/completions,"
  note "  unset KOHAKU_TEST_MOCK_LLM, and re-run this script."
elif [[ -z "${KOHAKU_TEST_LLAMA_URL:-}" ]]; then
  note "deferred: KOHAKU_TEST_LLAMA_URL not set"
  note "  to enable, run a loopback LLM (llama-server, vLLM, …) and"
  note "  export KOHAKU_TEST_LLAMA_URL=http://127.0.0.1:8080/v1/chat/completions"
else
  export KOHAKU_AGENT_LLM_URL="$KOHAKU_TEST_LLAMA_URL"
