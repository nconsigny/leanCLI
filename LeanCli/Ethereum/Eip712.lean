...
namespace LeanCli.Ethereum.Eip712
...
def encodeIntValue (width : Nat) (j : Json) : Except String ByteArray := do
  ...
  | .num n =>
      if n ≥ 0 then
        ...
        else .ok (zeroPad32 (LeanCli.Wallet.HDKey.Nat.toFixedBytes 32 nat))
      else
        ...
        if mag > bound then .error s!"int{width} value out of range"
          ...
          let twos := 2 ^ 256 - mag
          .ok (LeanCli.Wallet.HDKey.Nat.toFixedBytes 32 twos)
  | _ =>
      let n ← parseUintValue j
      let bound := 2 ^ (width - 1)
      if n ≥ bound then .error s!"int{width} value out of range"
      else .ok (zeroPad32 (LeanCli.Wallet.HDKey.Nat.toFixedBytes 32 n))
...
def encodeUintValue (width : Nat) (j : Json) : Except String ByteArray := do
  ...
  .ok (LeanCli.Wallet.HDKey.Nat.toFixedBytes 32 n)
