...
`LeanCli.Invariants.Ens.normalizeLabels` helper enforces this.
-/
...
private def hexNoPrefix (bytes : ByteArray) : String :=
  stripHexPrefix (LeanCli.Crypto.Hex.encode bytes)
...
def namehashIO (name : String) : IO (Except String ByteArray) := do
  match LeanCli.Invariants.Ens.normalizeLabels name with
...
def parseAddressWord (hex : String) : Option String := do
  let bytes ← LeanCli.Crypto.Hex.decode hex
  if bytes.size = 32 then
    ...
    some (LeanCli.Crypto.Hex.encode body)
  else
    none
...
def resolveIO (policy : Policy) (endpoint : LeanCli.RPC.Outbound.Endpoint)
  ...
  | some registry =>
      ...
      | .ok node =>
          ...
          match ← LeanCli.RPC.Outbound.ethCall policy endpoint registry resolverData "latest" via? with
              ...
              | some rHex =>
                  ...
                  | some resolverAddr =>
                      if resolverAddr = zeroAddressHex then
                        ...
                        match ← LeanCli.RPC.Outbound.ethCall policy endpoint resolverAddr addrData "latest" via? with
                            ...
                            | some aHex =>
                                ...
                                | some addrLower =>
                                    if addrLower = zeroAddressHex then
                                      ...
                                      match LeanCli.Ethereum.Address.fromHex addrLower with
                                      | none => pure (.error (-32020, s!"resolved address is malformed: {addrLower}"))
                                      | some address =>
                                          match ← LeanCli.Ethereum.Address.toChecksumIO address with
