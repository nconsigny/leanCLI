...
hard-rejects here — that's the job of `LeanCli/LlmAgent/IntentParser.lean`,
...
namespace LeanCli.Ethereum.IntentJson
...
def bytesField (obj : Json) (key : String) : Except String ByteArray := do
  ...
  match LeanCli.Crypto.Hex.decode s with
  | some b => .ok b
...
end LeanCli.Ethereum.IntentJson
