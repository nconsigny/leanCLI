...
`LeanCli.Swap.Tokens.registry` — the same list the wallet's SWAP
...
`LeanCli.Agent.ToolDefs.Tokens`) to obtain those fields, and the
answer is a pure Lean fact reviewed in `LeanCli.Swap.Tokens`.
...
namespace LeanCli.Ethereum.TokenRegistry
...
/-- The hand-audited token list, derived from
    `LeanCli.Swap.Tokens.registry`. The wallet daemon's swap
...
end LeanCli.Ethereum.TokenRegistry
