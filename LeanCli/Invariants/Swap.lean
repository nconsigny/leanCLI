...
namespace LeanCli.Invariants.Swap
...
def balancesCandidates (chain : LeanCli.Swap.Tokens.ChainId) :
    List (LeanCli.Swap.Tokens.Token × String) :=
  LeanCli.Swap.Tokens.registry.filterMap fun t =>
...
theorem balancesCandidates_addressOn_some
    (chain : LeanCli.Swap.Tokens.ChainId)
    (t : LeanCli.Swap.Tokens.Token) (addr : String)
    (h : (t, addr) ∈ balancesCandidates chain) :
    LeanCli.Swap.Tokens.addressOn t chain = some addr := by
  ...
  cases hAddr : LeanCli.Swap.Tokens.addressOn t' chain with
  | none =>
      simp [hAddr] at ht'eq
  | some a =>
      simp [hAddr] at ht'eq
