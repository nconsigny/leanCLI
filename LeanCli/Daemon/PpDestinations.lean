...
`$XDG_DATA_HOME/leancli/pp-destinations.ndjson`. Lookups are
linear in the file size, which is fine — a wallet typically
unshields tens of times over its life.
-/

namespace LeanCli.Daemon.PpDestinations
...
def storePath : IO System.FilePath := do
  pure ((← dataHome) / "leancli" / "pp-destinations.ndjson")
...
end LeanCli.Daemon.PpDestinations
