...
namespace LeanCli.Daemon.Status
...
private def sidecarJson (desc : SidecarDescriptor) : IO Json := do
  let resolved ← LeanCli.Util.BridgeResolve.resolveExecutable
...
private def sandboxJson : IO Json := do
  let mode := ((← IO.getEnv "LEANCLI_SANDBOX").getD "auto")
  let usable ← LeanCli.Util.Sandbox.probeUsableUnshare
...
private def daemonJson (state : LeanCli.Daemon.State.Shared) : IO Json := do
...
/-- Resolve the recorded checkout root via `$LEANCLI_HOME/checkout` (the
    marker `leanclispawn` writes). Falls back to the daemon's CWD if the
...
private def resolveCheckoutRoot : IO (Option System.FilePath) := do
  ...
  match (← IO.getEnv "LEANCLI_CHECKOUT") with
  | some s =>
      let p := System.FilePath.mk s
      if (← (p / "lakefile.lean").pathExists) then return some p
      else return none
  | none =>
      let leancliHome ← match (← IO.getEnv "LEANCLI_HOME") with
        | some s => pure (System.FilePath.mk s)
        | none =>
            match (← IO.getEnv "HOME") with
            | some h => pure ((System.FilePath.mk h) / ".leancli")
            | none   => pure (System.FilePath.mk ".leancli")
      let indexFile := leancliHome / "checkout"
...
/-- Wallet posture sketch — initialized?, master unlocked?, count of
    ...
    Status page atomic. -/
private def walletJson (state : LeanCli.Daemon.State.Shared) : IO Json := do
  let unlocked ← LeanCli.Daemon.State.unlockedNames state
...
private def hostNoPort (url : String) : String := Id.run do
  let host := LeanCli.RPC.Outbound.hostOfUrl url
...
private def endpointJson (ep : LeanCli.RPC.Outbound.Endpoint) : IO Json := do
...
private def chainEndpointJson
    (activeChainId : Nat)
    (entry : String × LeanCli.RPC.Outbound.Endpoint) : IO Json := do
  let (name, ep) := entry
  let host := hostNoPort ep.url
  let ip ← resolveIpv4 host
  let (egressSrc, egressDev, egressError) ← routeEgress ip
  let resolvedChainId : Option Nat :=
    ep.chainId.orElse (fun _ => LeanCli.RPC.Outbound.chainNameToId name)
...
def buildSnapshot
    ...
    (ensRpcEndpoint : Option LeanCli.RPC.Outbound.Endpoint)
