...
`leancli-agent` executable; this module defines what a single
one-shot agent invocation looks like in memory.

Trust contract: nothing in `LeanCli/Agent/**` imports any of the
...
structure AgentConfig where
  /-- Loopback URL of the OpenAI-compatible chat endpoint — must
      satisfy `LeanCli.Agent.Http.isLoopbackUrl`. -/
  llmUrl         : String
