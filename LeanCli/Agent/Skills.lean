...
`LeanCli/Daemon/SkillsStore.lean`. The Daemon module exposes
...
namespace LeanCli.Agent.Skills
...
structure SkillFrontmatter where
  ...
  /-- Always include in the system prompt when present. Used by the
      meta-skills (`leancli-wallet`, `web3-security`). -/
  alwaysOn    : Bool := false
