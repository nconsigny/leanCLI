...
namespace LeanCli.RPC.Outbound
...
error if `LEANCLI_RPC_URL` is unset/empty. The daemon proper uses
`LeanCli.Daemon.Config.resolve` which also reads `daemon.json`; this
helper exists for ad-hoc IO sites. No localhost fallback. -/
def resolveEndpoint : IO Endpoint := do
  let url ← match ← IO.getEnv "LEANCLI_RPC_URL" with
    | some url =>
        let trimmed := url.trimAscii.toString
        if trimmed.isEmpty then
          throw <| IO.userError
            "no rpc_url configured: set LEANCLI_RPC_URL (empty value rejected)"
        else
          pure trimmed
    | none =>
        throw <| IO.userError
          "no rpc_url configured: set LEANCLI_RPC_URL or 'rpc_url' in daemon.json"
  let transport? ←
    match ← IO.getEnv "LEANCLI_RPC_TRANSPORT" with
...
def providerConfig (endpoint : Endpoint) : LeanCli.Network.Provider.Config :=
...
private def verboseLevel : IO Nat := do
  match ← IO.getEnv "LEANCLI_VERBOSE" with
  | some s => pure (s.toNat?.getD 0)
  | none => pure 0

/-- Path for appending a JSONL network event log. Enabled by default at
`$XDG_STATE_HOME/leancli/network.log` (or `$HOME/.local/state/...`).
`LEANCLI_NETWORK_LOG` overrides:
  ...
  let defaultPath : IO String := do
    let base ←
      ...
      | none =>
          ...
          | none => pure "/tmp"
    pure s!"{base}/leancli/network.log"
  match ← IO.getEnv "LEANCLI_NETWORK_LOG" with
...
endpoint resolved at startup from `LEANCLI_RPC_URL` / `daemon.json`).
...
def call (policy : Policy) (endpoint : Endpoint) (method : RpcMethod)
  ...
  if endpoint.url.trimAscii.toString.isEmpty then
    return .error "no rpc_url configured: refusing to dial (set LEANCLI_RPC_URL or 'rpc_url' in daemon.json)"
  ...
  logEvent "request" method.asString params
    #[("url", .str endpoint.url),
      ...
      ("params", params)]
  try
    let raw ← LeanCli.RPC.JsonRpc.callRawDetailed endpoint.url
