...
namespace LeanCli.LlmAgent.Bridge

open LeanCli.Encoding.Json

def defaultExecutable : String := "leancli-agent"
  ...
  let overrideOk? : IO (Option String) := do
    ...
    | some s =>
        if s.contains '/' ∧ !(← System.FilePath.pathExists (System.FilePath.mk s)) then do
          IO.eprintln s!"[LEANCLI_LLM_BRIDGE] override points at missing file ({s}); falling back to default lookup chain"
          pure none
        else
          pure (some s)
...
/-- Resolve the agent UDS socket path. Mirrors
    `leancli-agentd`'s own `resolveAgentSocket`. -/
...
/-- Send a single newline-delimited JSON frame on `socketPath` and
    read the reply line. The peer is `leancli-agentd`, which always
...
private def socketCall (socketPath : String) (frame : String) :
  ...
  try
    let conn ← LeanCli.Transport.Uds.connect socketPath
    try
      let _ ← LeanCli.Transport.Uds.write conn (frame ++ "\n").toByteArray
...
(peer likely crashed mid-handler; check `journalctl -u leancli-agentd` \
...
private def callPersistent (req : Request) : IO Response := do
  ...
  | "llm.parseIntent" =>
      ...
      let incognito : Bool ← do
        match ← IO.getEnv "LEANCLI_INCOGNITO" with
        | none => pure false
        | some v =>
            let t := v.trimAscii.toString
            pure (t ≠ "" && t ≠ "0")
...
/-- Read-only session listing. Forwards `{limit, chainId, sessionKey}`
    ...
    a `result.sessions` array; see `LeanCli/App/AgentDaemonMain.lean`
    for the field set. -/
