...
intents. Replaces the regex matcher that lived in the now-removed Node
LLM sidecar. Lean is a strict win here: no regex ambiguity, total
functions, the parser output IS an ADT, every match arm typechecks.
