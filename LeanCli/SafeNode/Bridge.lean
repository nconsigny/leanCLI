...
This module mirrors `LeanCli.Helios.Bridge` 1:1 (same `Request` /
...
`LeanCli.SafeNode.Persistent`.
-/

namespace LeanCli.SafeNode.Bridge
...
/-- Resolve the bridge executable. `LEANCLI_SAFENODE_BRIDGE`
    overrides for local development. -/
def resolveExecutable : IO String := do
  match (← IO.getEnv "LEANCLI_SAFENODE_BRIDGE") with
  | some s => pure s
...
/-- Spawn the sidecar for one request, write the encoded request as
    ...
    pattern as `LeanCli.Helios.Bridge.call`. -/
