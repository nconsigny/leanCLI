...
to the daemon. `LEANCLI_PASSPHRASE` and `LEANCLI_PIN` are accepted for
scripted tests; the interactive fallback disables terminal echo when possible.
-/

namespace LeanCli.Cli.Passphrase
...
def read (label : String := "Passphrase: ") : IO String :=
  readSecret "LEANCLI_PASSPHRASE" label

end LeanCli.Cli.Passphrase

namespace LeanCli.Cli.Pin

/-- Read a TPM PIN from the terminal with echo disabled. Falls back to the
    `LEANCLI_PIN` env var when set, so scripted tests can drive the flow
    without poking the terminal. -/
def read (label : String := "PIN: ") : IO String := do
  match ← IO.getEnv "LEANCLI_PIN" with
  | some pin => pure pin
  | none =>
      try
        let child ← IO.Process.spawn
