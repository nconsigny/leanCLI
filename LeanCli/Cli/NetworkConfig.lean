...
def resolved : IO (String × String × String × String × String) := do
  ...
  let urlOpt ← envOr "LEANCLI_RPC_URL" ["rpc_url", "rpcUrl", "rpc_endpoint", "rpcEndpoint"]
  let url := urlOpt.getD "<unset>"
  let transportS := (← envOr "LEANCLI_RPC_TRANSPORT" ["rpc_transport", "rpcTransport"]).getD "<auto>"
  let policyS := (← envOr "LEANCLI_NETWORK_POLICY" ["network_policy", "networkPolicy"]).getD "strict"
...
/-- Persist `chain_id` in `daemon.json`. Used by `leancli network set-chain`
...
def networkLogPath : IO (Option String) :=
  LeanCli.RPC.Outbound.networkLogPath
...
def humanReport : IO String := do
  ...
  -- `{ url, transport }` object forms (matches `LeanCli.Daemon.Config`).
    ...
    fileFields.find? (fun (k, _) => k = "rpc_urls") |>.bind fun (_, v) =>
      ...
      | .obj inner =>
          inner.find? (fun (k, _) => k = chain) |>.bind fun (_, entry) =>
            ...
            | _ => none
      | _ => none
  -- ENS falls back to rpc_urls.mainnet so `leancli network set-rpc-chain
  -- mainnet <url>` alone is enough to enable ENS resolution. Mirrors the
  -- daemon-side resolver in `LeanCli/Daemon/Config.lean`.
  let fileEns :=
  ...
  let envRpc ← IO.getEnv "LEANCLI_RPC_URL"
  -- Same fallback chain as the daemon: namespaced env > generic env.
  let trim? (s : String) : Option String :=
    let t := s.trimAscii.toString
    if t.isEmpty then none else some t
  let envEnsRaw ← IO.getEnv "LEANCLI_ENS_RPC_URL"
  let envEnsNs ← IO.getEnv "LEANCLI_RPC_URL_MAINNET"
  let envEnsGen ← IO.getEnv "MAINNET_RPC_URL"
  let envEns := (envEnsRaw.bind trim?).orElse fun () =>
                  (envEnsNs.bind trim?).orElse fun () =>
                    (envEnsGen.bind trim?)
  let envTransport ← IO.getEnv "LEANCLI_RPC_TRANSPORT"
  let envPolicy ← IO.getEnv "LEANCLI_NETWORK_POLICY"
  let envSocket ← IO.getEnv "LEANCLI_SOCKET"
  let envChainId ← IO.getEnv "LEANCLI_CHAIN_ID"
  ...
  let envChainUrl (chain : String) : IO (Option (String × String)) := do
    ...
    let trim? (raw : String) : Option String :=
      ...
      if t.isEmpty then none else some t
    match (← IO.getEnv s!"LEANCLI_RPC_URL_{upper}").bind trim? with
    | some v => pure (some (v, s!"env: LEANCLI_RPC_URL_{upper}"))
    | none =>
  ...
  let chainsBlock : String :=
    if rows.isEmpty then
      "  per-chain rpc:  <none configured — set via `leancli network set-rpc-chain <name> <url>` or " ++
  ...
  let logLine :=
    match logPath with
    | some p => s!"network log: {p}  (default-on; LEANCLI_NETWORK_LOG=0 disables)"
    | none => "network log: <disabled via LEANCLI_NETWORK_LOG=0>"
  pure <|
    "leanCLI networking config\n" ++
    s!"  config file:    {path}\n" ++
    ...
    s!"  {logLine}\n" ++
    "  env overrides:  LEANCLI_RPC_URL, LEANCLI_RPC_TRANSPORT, LEANCLI_NETWORK_POLICY,\n" ++
    "                  LEANCLI_CHAIN_ID, LEANCLI_SOCKET, LEANCLI_NETWORK_LOG,\n" ++
    "                  LEANCLI_ENS_RPC_URL\n" ++
    "  per-chain env:  LEANCLI_RPC_URL_<NAME>  (authoritative)\n" ++
...
def currentChainName : IO String := do
  match ← IO.getEnv "LEANCLI_CHAIN_ID" with
  | some "1" => pure "mainnet"
