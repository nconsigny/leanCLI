...
namespace LeanCli.Cli.DaemonClient
...
/-- Resolve the per-user runtime directory hosting the wallet UDS
    socket. Mirrors `LeanCli.Daemon.Config.runtimeDir`: prefers
...
def autoSpawnDisabled : IO Bool := do
  match ← IO.getEnv "LEANCLI_NO_AUTOSPAWN" with
  | none => pure false
  | some "" => pure false
  | some "0" => pure false
...
/-- Path of the "this machine is managed by systemd" marker dropped by
    `leanclispawn`. The presence of this file disables autospawn so the
    ...
    back to `$HOME/.config/leancli/managed-by-systemd`. The function
...
def systemdMarkerPath : IO String := do
  ...
  pure s!"{cfgRoot}/leancli/managed-by-systemd"
...
/-- Spawn the daemon with stderr piped back to us. We keep the child handle
    ...
    writes to `$XDG_STATE_HOME/leancli/network.log`, not stderr, so on
...
partial def waitForSocketConnect (path : String) (remaining : Nat) : IO Bool := do
  if remaining == 0 then
    ...
    try
      let conn ← LeanCli.Transport.Uds.connect path
      LeanCli.Transport.Uds.close conn
      pure true
    catch _ =>
      IO.sleep 100
...
CLI is the "no rpc_url configured" case (LeanCli/Daemon/Config.lean):
...
this, `leancli daemon ping` on a fresh box says exactly what to fix. -/
def ensureDaemon (path method : String) : IO (Except String Unit) := do
  if noAutoSpawnMethod method then
    ...
    pure (.error "autospawn disabled (LEANCLI_NO_AUTOSPAWN is set)")
  else if ← systemdManaged then
    -- leanclispawn dropped the marker; the daemon's lifecycle is now
    ...
    pure (.error
      s!"leancli-daemon is managed by systemd on this machine ({marker} present).\nStart it with:  leancli daemon start\nTail logs with: leancli daemon logs")
  else
...
def callOnce (path method : String) (params : Json := .arr #[]) : IO (Except RpcError Json) := do
  let conn ← LeanCli.Transport.Uds.connect path
  try
    discard <| LeanCli.Transport.Uds.write conn (compact (requestJson method params) ++ "\n").toByteArray
