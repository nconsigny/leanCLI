...
Mirrors `LeanCli.Colibri.Bridge` 1:1 (same `Request` / `Response` /
...
namespace LeanCli.Helios.Bridge
...
/-- Resolve the bridge executable. The `LEANCLI_HELIOS_BRIDGE`
    environment variable overrides for local development. -/
def resolveExecutable : IO String := do
  match (← IO.getEnv "LEANCLI_HELIOS_BRIDGE") with
  | some s => pure s
...
/-- Spawn the sidecar for one request, write the encoded request as
    ...
    pattern as `LeanCli.Colibri.Bridge.call`. Helios's first
