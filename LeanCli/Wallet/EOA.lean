...
namespace LeanCli.Wallet.EOA

open LeanCli.Crypto
open LeanCli.Crypto.Secp256k1
open LeanCli.Ethereum.Tx
...
def signingDigestIO (tx : TxEip1559) : IO (Except String ByteArray) :=
  Hacl.keccak256EthereumIO (LeanCli.Crypto.Hex.encode tx.signingPayload)
...
def signDigestIO (privateKey digest : ByteArray) : IO (Except String Signature) := do
  if privateKey.size != 32 then
    ...
    let sig ← Secp256k1Native.signIO
      (LeanCli.Crypto.Hex.encode privateKey)
      (LeanCli.Crypto.Hex.encode digest)
    match sig with
...
def signPersonalMessageIO (msg privateKey : ByteArray) :
  ...
  let digest ← Hacl.keccak256EthereumIO
    (LeanCli.Crypto.Hex.encode (personalMessagePayload msg))
  match digest with
  | .error err => pure (.error err)
  | .ok digestBytes => signDigestIO privateKey digestBytes

end LeanCli.Wallet.EOA
