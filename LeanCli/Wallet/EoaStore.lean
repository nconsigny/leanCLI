...
namespace LeanCli.Wallet.EoaStore
...
structure Record where
  ...
  -- wallet-level master KEK (see `LeanCli.Keystore.MasterPassphrase`).
...
private def sealMnemonic (key : ByteArray) (name derivationPath address phrase : String) :
  ...
  match ← LeanCli.Crypto.Hacl.chacha20Poly1305SealIO
      ...
      (LeanCli.Crypto.Hex.encode (mnemonicAad name derivationPath address))
      (LeanCli.Crypto.Hex.encode phrase.toByteArray) with
...
def makeRecord (name passphrase : String) (seed : ByteArray)
  ...
  | .error err => pure (.error err)
  | .ok keyBytes =>
      let sealed ← LeanCli.Crypto.Hacl.chacha20Poly1305SealIO
        (LeanCli.Crypto.Hex.encode keyBytes)
        (LeanCli.Crypto.Hex.encode nonce)
        (LeanCli.Crypto.Hex.encode (aad name derivationPath address))
        (LeanCli.Crypto.Hex.encode seed)
      match sealed with
      | .error err => pure (.error err)
      | .ok ciphertext =>
...
def unlockSeedIO (record : Record) (passphrase : String) : IO (Except String ByteArray) := do
  ...
  | .ok keyBytes =>
      LeanCli.Crypto.Hacl.chacha20Poly1305OpenIO
        (LeanCli.Crypto.Hex.encode keyBytes)
        (LeanCli.Crypto.Hex.encode record.aeadNonce)
        (LeanCli.Crypto.Hex.encode (aad record.name record.derivationPath record.address))
        (LeanCli.Crypto.Hex.encode record.ciphertext)
...
def wrapWithMaster (masterKey : ByteArray) (slotName : String) (seed : ByteArray) :
  ...
  let nonce ← LeanCli.Crypto.Random.getRandomBytes 12
  match ← LeanCli.Crypto.Hacl.chacha20Poly1305SealIO
      (LeanCli.Crypto.Hex.encode masterKey)
      (LeanCli.Crypto.Hex.encode nonce)
      (LeanCli.Crypto.Hex.encode (attestationAad slotName))
