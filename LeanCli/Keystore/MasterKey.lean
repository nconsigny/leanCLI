...
(`.leancli/keystore/tpm2/_master`) and the sealed key ended up wherever
`leancli wallet master init` was run from; the master.json manifest is and
...
def masterDir : IO System.FilePath := do
  pure ((← LeanCli.Wallet.EoaStore.dataHome) / "leancli" / "keystore" / "tpm2" / "_master")
...
private def manifestText : String :=
  "leancli TPM2 master attestation key\n" ++
...
private def hardenMasterDir : IO Unit := do
  -- Walk up from the master dir and harden each ancestor under leancli/.
  -- Built explicitly rather than via splitOn so we keep the absolute form.
  let dir ← masterDir
  -- dir = <dataHome>/leancli/keystore/tpm2/_master
  let tpm2 := dir.parent.getD dir
  let keystore := tpm2.parent.getD tpm2
  let leanCLI := keystore.parent.getD keystore
  for p in [leanCLI, keystore, tpm2, dir] do
    hardenDir p
...
def bootstrap (pin : String) (notify : Notifier) : IO (Except String Unit) := do
  ...
  let seedBytes ← LeanCli.Crypto.Random.getRandomBytes 32
...
end LeanCli.Keystore.MasterKey
