...
four resolvers. The resolution order is:
  ...
  3. `$LEANCLI_HOME/checkout` (default `$HOME/.leancli/checkout`) — a
     marker file `script/leanclispawn` writes that records the absolute
  ...
  4. `defaultExe` — a PATH-resolved fallback name like
     `leancli-clearsign-bridge`. `script/leanclispawn` installs
     wrapper shims under `$LEANCLI_HOME/bin/` for this case.
-/

namespace LeanCli.Util.BridgeResolve
...
/-- Read the checkout path that `script/leanclispawn` writes to
    `$LEANCLI_HOME/checkout` (default `$HOME/.leancli/checkout`) and return
...
/-- A "path-like" value is anything that contains a `/`. PATH-resolved
    bare names (e.g. `leancli-agent`, `leancli-clearsign-bridge`) are
...
end LeanCli.Util.BridgeResolve
