...
def main : IO UInt32 := do
  ...
  | .ok td =>
      ...
      | .error err =>
          ...
          return 1
      | .ok d =>
          let digestHex := LeanCli.Crypto.Hex.encode d.digest
          IO.println s!"primaryType: {d.primaryType}"
          IO.println s!"domainSeparator: {LeanCli.Crypto.Hex.encode d.domainSeparator}"
          IO.println s!"messageHash: {LeanCli.Crypto.Hex.encode d.messageHash}"
