...
NIST variant (`slhDsaSha2_128_24`) and the WOTS+C / FORS+C C13 variant
must both succeed end-to-end (keygen → sign → verify-after-sign).

Used by humans / CI as:

    lake script run sphincs-shims                      # build binaries
    lake env .lake/build/bin/leancli-sphincs-test   # run the smoke
...
def main : IO UInt32 := do
  ...
  -- C13 takes a 32-byte seed (truncated from the same source the SLH-DSA
  -- variant uses, so both runs are reproducible from the file's literal).
  -- NOTE: the `sphincs-c13` signer binary is a native build artifact
  -- (see `sidecars/sphincs/`); until it is built this roundtrip fails
  -- closed at the spawn step, which is the intended behaviour.
  let c13Seed : String := (slhdsaSeedHex.take 64).toString
  let okC13 ← runSmoke "C13" ParamSet.c13 c13Seed (expectStub := false)
  pure (if okSlh && okC13 then 0 else 1)
