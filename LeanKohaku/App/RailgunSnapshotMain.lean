...
How it works:
  ...
  2. Spawn the kohaku bridge with `shielded.railgun.balance` against
...
Run as a one-shot dev tool when refreshing the snapshot for a release:
  ...
  lake build leankohaku-railgun-snapshot
...
private def tmpStorageDir : System.FilePath := "/tmp/leankohaku-railgun-snapshot"
...
def main (_args : List String) : IO UInt32 := do
  ...
  let env : Array (String × Option String) := #[
    ("LEANKOHAKU_RPC_URL", some rpcUrl),
    ("LEANKOHAKU_CHAIN_ID", some "11155111"),
    ("LEANKOHAKU_RG_SEED_HEX", some deterministicSeedHex),
    ("LEANKOHAKU_RG_STORAGE_PATH", some storagePath),
    -- Disable the cold-start seed-from-bundle so we generate a fresh
    -- snapshot via real sync, not a copy of any existing bundle.
    ("LEANKOHAKU_RG_SNAPSHOT_DISABLE", some "1")
  ]
  ...
  let req : LeanKohaku.Privacy.Bridge.Request :=
    { method := "shielded.railgun.balance", params := .obj #[], id := 0 }
  let resp ← LeanKohaku.Privacy.Bridge.callWithEnv req env
