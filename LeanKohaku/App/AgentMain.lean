...
namespace LeanKohaku.App.AgentMain
...
/-- Resolve the daemon socket path. Preference order:
    1. `LEAN_KOHAKU_DAEMON_SOCKET` (agent-specific override).
    2. `LEANKOHAKU_SOCKET` (existing CLI/TUI convention).
    3. `<XDG_RUNTIME_DIR>/leankohaku/leankohaku.sock`
...
def main (args : List String) : IO UInt32 := do
  ...
  | none =>
      IO.eprintln "usage: kohaku-agent --rpc '<json-rpc-request>'"
      pure 2
  | some raw =>
...
end LeanKohaku.App.AgentMain

def main (args : List String) : IO UInt32 :=
  LeanKohaku.App.AgentMain.main args
