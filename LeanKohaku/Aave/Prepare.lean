...
Mirrors `LeanKohaku/Swap/Prepare.lean` for the Aave V3 Pool. A single
...
namespace LeanKohaku.Aave.Prepare

open LeanKohaku.Encoding.Json (Json)
open LeanKohaku.Swap
open LeanKohaku.Aave
open LeanKohaku.Wallet.ExecuteBatch (AccountKindHint encodeExecuteBatch)
...
def fmtAmount (n : Nat) (decimals : Nat) : String :=
  match LeanKohaku.Ethereum.TokenRegistry.humanUnits (toString n) decimals with
  | .ok s => s
  | .error _ => toString n
...
def resolveContext
  ...
  let some _ := LeanKohaku.Ethereum.Address.fromHex sender
    | .error <| .err "invalid_address"
        s!"sender is not a 0x-prefixed 20-byte address: {sender}"
  let some _ := LeanKohaku.Ethereum.Address.fromHex secondAddr
    | .error <| .err "invalid_address"
...
def readAllowance
  ...
  let data := LeanKohaku.Swap.UniV3.encodeAllowance owner spender
...
def attachAllowance
  ...
  if currentAllowance ≥ required then
    ...
    let approveData :=
      LeanKohaku.Swap.UniV3.encodeApprove pool LeanKohaku.Swap.UniV3.maxUint256
    let approve : TxFrame :=
...
private def txFrameToCall (f : TxFrame) :
    LeanKohaku.Wallet.ExecuteBatch.Call :=
