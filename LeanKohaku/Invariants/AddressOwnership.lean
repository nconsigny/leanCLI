...
namespace LeanKohaku.Invariants.AddressOwnership

open LeanKohaku.Ethereum.Ownership

/-- 🔒 FFI: BIP-44 Ethereum address derived from a seed at a path.
    Implemented by `LeanKohaku.Wallet.HDKey` (HMAC-SHA512 / secp256k1)
    + `LeanKohaku.Wallet.Address` (Keccak-256 / EIP-55). Axiomatized
...
/-- Daemon view of an EOA record on disk. Mirrors
    `LeanKohaku.Wallet.EoaStore.Record` projected to the fields the
    resolver uses. -/
...
/-- In-memory unlocked-wallet slot. Mirrors
    `LeanKohaku.Daemon.State.UnlockedSlot`. -/
...
end LeanKohaku.Invariants.AddressOwnership
