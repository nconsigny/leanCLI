...
Reads the kohaku skills pack at `<project root>/skills/` so the daemon
...
Default: `<cwd>/skills/`. Override with `LEAN_KOHAKU_SKILLS_DIR`. This
...
namespace LeanKohaku.Daemon.SkillsStore
...
def skillsRoot : IO System.FilePath := do
  match ← IO.getEnv "LEAN_KOHAKU_SKILLS_DIR" with
  | some d => pure d
...
end LeanKohaku.Daemon.SkillsStore
