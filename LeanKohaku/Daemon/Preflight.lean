...
namespace LeanKohaku.Daemon.Preflight

open LeanKohaku.Encoding.Json
open LeanKohaku.Util.Units
...
`KOHAKU_PREFLIGHT_LOOKBACK` for indexed providers. -/
    ...
    let rec toHex (k : Nat) (acc : String) (fuel : Nat) : String :=
      ...
      | fuel + 1 =>
          if k = 0 then acc
            ...
            let d := k % 16
            let c := LeanKohaku.Crypto.Hex.nibbleToChar (UInt8.ofNat d)
...
private def paddedAddrTopic (addr : String) : String :=
  "0x" ++ LeanKohaku.Swap.UniV3.encodeAddress addr
...
private def renderAmount (amt : Nat) (meta? : Option LeanKohaku.Daemon.TokenMeta.TokenMeta) : String :=
  match meta? with
...
sentinel. -/
private def renderApproveAmount (amt : Nat) (meta? : Option LeanKohaku.Daemon.TokenMeta.TokenMeta) : String :=
...
private def readAllowance
    ...
    (via? : Option LeanKohaku.RPC.Outbound.VerifyVia)
    (token owner spender : String) : IO (Option Nat) := do
  let data := selAllowance ++ LeanKohaku.Swap.UniV3.encodeAddress owner ++ LeanKohaku.Swap.UniV3.encodeAddress spender
  match ← LeanKohaku.RPC.Outbound.ethCall policy endpoint token data "latest" via? with
  | .ok j =>
...
private def readErc20Balance
  ...
  let data := selBalanceOf ++ LeanKohaku.Swap.UniV3.encodeAddress owner
  match ← LeanKohaku.RPC.Outbound.ethCall policy endpoint token data "latest" via? with
  | .ok j =>
      match asString j with
      | some s => pure (parseHexQuantity s)
...
private def readNativeBalance
  ...
  match ← LeanKohaku.RPC.Outbound.getBalance policy endpoint owner "latest" via? with
  | .ok j =>
      match asString j with
      | some s => pure (parseHexQuantity s)
      | none => pure none
...
private def readBlockNumber
    (policy : LeanKohaku.Privacy.NetworkPolicy.Policy)
    (endpoint : LeanKohaku.RPC.Outbound.Endpoint)
    (via? : Option LeanKohaku.RPC.Outbound.VerifyVia) : IO (Option Nat) := do
  match ← LeanKohaku.RPC.Outbound.blockNumber policy endpoint via? with
  | .ok j =>
      match asString j with
      | some s => pure (parseHexQuantity s)
      | none => pure none
...
private def countPriorEvents
  ...
  | some head =>
      ...
      match ← LeanKohaku.RPC.Outbound.getLogs policy endpoint
...
def run
  ...
  | .transfer recipient amount =>
      let metaOpt ← LeanKohaku.Daemon.TokenMeta.lookupOrFetch state policy endpoint chainId to
...
end LeanKohaku.Daemon.Preflight
