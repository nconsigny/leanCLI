...
`$XDG_DATA_HOME/leankohaku/addressbook.json` (else
`~/.local/share/leankohaku/addressbook.json`). Same parent dir as the
...
namespace LeanKohaku.Daemon.AddressBook
...
def bookPath : IO System.FilePath := do
  pure ((← dataHome) / "leankohaku" / "addressbook.json")
...
def loadIO : IO (Except String (List Entry)) := do
  ...
  match LeanKohaku.Encoding.Json.parse content with
...
def saveIO (entries : List Entry) : IO (Except String Unit) := do
  try
    ...
    IO.FS.writeFile tmp (LeanKohaku.Encoding.Json.pretty body)
    IO.FS.rename tmp p
    pure (.ok ())
...
end LeanKohaku.Daemon.AddressBook
