...
import LeanKohaku.Agent.MemoryPrompts
...
* Path: `$XDG_DATA_HOME/leankohaku/MEMORY.md`, with the standard
...
/-- Resolve the canonical MEMORY.md path. Honours
    `KOHAKU_AGENT_MEMORY` (testing override) then
    `$XDG_DATA_HOME/leankohaku/MEMORY.md`, falling back to
    `$HOME/.local/share/leankohaku/MEMORY.md`, finally `/tmp`. -/
...
def estimateTokens (text : String) : IO Nat := do
  let (num, den) ← do
    match ← IO.getEnv "KOHAKU_TOKEN_RATIO" with
    | none => pure (14, 10)
    | some s =>
