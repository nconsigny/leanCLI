...
ready/needs_approval branching — see `LeanKohaku/Aave/Prepare.lean`.
...
namespace LeanKohaku.Agent.ToolDefs.Aave
...
/-- 2^256 - 1, the Aave "MAX (full balance)" sentinel. Mirror of
    `LeanKohaku.Aave.Prepare.maxUint256` lifted to the agent layer so we
    can keep `LeanKohaku.Aave.*` out of the agent import set (the
...
end LeanKohaku.Agent.ToolDefs.Aave
