...
/-!
# `kohaku memory` CLI subcommands

Four read/write entrypoints that talk to the running `kohaku-agentd`
...
private def memoryPath : IO System.FilePath := do
  match ← IO.getEnv "KOHAKU_AGENT_MEMORY" with
  ...
  | none =>
      ...
      pure (data / "leankohaku" / "MEMORY.md")
...
/-- `kohaku memory show` — print MEMORY.md to stdout. Reads
...
/-- `kohaku memory edit` — open MEMORY.md in `$EDITOR` (fallback
...
/-- `kohaku memory refresh [--session N]` — force-extract from
...
/-- `kohaku memory forget <pattern>` — drop every line containing
