...
Sealing model (v1):
  ...
  * Optional path — `masterWrap`: the same plaintext re-wrapped under
    ...
    (`LeanKohaku.Keystore.MasterPassphrase`). When present and the
...
namespace LeanKohaku.Wallet.SphincsHybridStore
...
structure Record where
  ...
  name             : String
  /-- Parameter set: emitted using `LeanKohaku.Sphincs.ParamSet.toString`
  ...
  ecdsaAttachment  : LeanKohaku.Wallet.Account.EcdsaAttachment
  ...
  ownerAddress     : String
  /-- SPHINCS- public seed and root (hex). For C9 these are 32-byte ABI
      words with the meaningful 16 bytes in the high half; for
      SLH-DSA-SHA2-128-24 and JARDIN-Keccak-128-24 they are 16-byte
      half-words. The Bridge's `KeyMaterial` shape governs. -/
  pkSeed           : String
  pkRoot           : String
...
def storeDir : IO System.FilePath := do
  pure ((← dataHome) / "leankohaku" / "sphincs-hybrid")
...
def ecdsaAttachmentToJson :
    LeanKohaku.Wallet.Account.EcdsaAttachment → Json
  | .existing wallet idx =>
      .obj #[
        ("kind",         .str "existing"),
...
def derivationPathFromString (s : String) :
    Except String LeanKohaku.Wallet.Account.DerivationPath := do
...
def ecdsaAttachmentFromJson (json : Json) :
    Except String LeanKohaku.Wallet.Account.EcdsaAttachment := do
...
def Record.fromJson (json : Json) : Except String Record := do
  ...
  if version ≠ currentVersion then
    ...
    let paramSet ← match LeanKohaku.Sphincs.ParamSet.parse? paramSetStr with
      | some p => .ok p
...
def sealSk (name : String) (paramSet : LeanKohaku.Sphincs.ParamSet)
  ...
  | .ok key =>
      ...
      let skBytes := match LeanKohaku.Crypto.Hex.decode skHex with
        | some bs => bs
      ...
      if skBytes.size = 0 then
        ...
        match ← LeanKohaku.Crypto.Hacl.chacha20Poly1305SealIO
            ...
            (LeanKohaku.Crypto.Hex.encode (aad name paramSet.toString ownerAddress))
            (LeanKohaku.Crypto.Hex.encode skBytes) with
        | .error err => pure (.error err)
...
def openSk (rec : Record) (passphrase : String) :
  ...
  if rec.passphraseCiphertext.size < 12 then
    ...
    | .ok key =>
        ...
        | .ok skBytes => pure (.ok (LeanKohaku.Crypto.Hex.encode skBytes))
...
def derivPathAad (paramSet : LeanKohaku.Sphincs.ParamSet) : String :=
  "sphincs-hybrid:" ++ paramSet.toString

/-- Seal a SPHINCS- sk under the daemon's loaded master KEK. The caller
    must obtain `kek` from `LeanKohaku.Daemon.State.getMasterKek?`. -/
def sealUnderMaster (kek : ByteArray) (name : String)
    (paramSet : LeanKohaku.Sphincs.ParamSet) (ownerAddress skHex : String) :
    IO (Except String ByteArray) := do
  match LeanKohaku.Crypto.Hex.decode skHex with
  | none => pure (.error "sealUnderMaster: sk hex did not decode")
  | some skBytes =>
      LeanKohaku.Keystore.MasterPassphrase.wrapSlot
...
end LeanKohaku.Wallet.SphincsHybridStore
