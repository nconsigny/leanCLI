...
`--listen` mode. The one-shot `LeanKohaku.Helios.Bridge.call` (--rpc)
remains for ad-hoc scripts but is not on the read path.
-/

namespace LeanKohaku.Helios.Persistent
...
end LeanKohaku.Helios.Persistent
