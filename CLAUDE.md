...
`Helios/Persistent.lean` is the parallel boundary for `@a16z/helios`. Same UDS / newline-JSON wire protocol; same trust posture (output is rendered to ConfirmGate, never trusted for signing). Toggled via `daemon.helios.toggle`; opt-in at boot via `KOHAKU_HELIOS=1` (since `@a16z/helios` must be installed under `bridge/helios/` first). RPCs: `tx.simulateHelios`, `eth.proxyHelios`, `daemon.helios.status`. The sidecar injects `executionRpc` and `chainId` from `cfg.rpcEndpoint` automatically and auto-fetches a fresh finalized checkpoint from the supplied `consensusRpc` (no caller-side plumbing). `consensusRpc` (beacon API) is caller-supplied with a `lightclientdata.org` default for mainnet only.

**Upstream blocker (as of 2026-05): `@a16z/helios@0.11.1` (Feb 2026, latest on npm) predates the Fulu hard fork.** Mainnet and Sepolia beacon endpoints now return `"version":"fulu"`-encoded light-client data that helios's WASM cannot parse, surfacing as `sync failed: invalid sync committee period`. The leanKohaku side of the integration is structurally complete and verified end-to-end; it will start producing real simulations the moment a Fulu-aware helios release ships. Until then, `daemon.helios.status` works, dispatch through to the sidecar works, but `waitSynced()` fails.
