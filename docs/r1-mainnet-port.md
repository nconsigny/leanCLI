...
- **Lean precompile model** — `LeanKohaku/Ethereum/P256Precompile.lean:18,22`
...
- **TPM signing primitive** — `signSepoliaDigest` in
  `LeanKohaku/Keystore/Tpm2Runtime.lean:450` is misnamed; the implementation
...
- **Network policy** — `LeanKohaku/Privacy/NetworkPolicy.lean` is already
...
- **Daemon chain selector** — accepts `mainnet` for EOA flows already
  (`LeanKohaku/Cli/Runtime.lean:1894`).
...
2. **Daemon RPC methods** — `LeanKohaku/Daemon/Server.lean`
...
4. **CLI help text & docs**
   - `LeanKohaku/Cli/Commands.lean:779` — "sepolia today; mainnet is
     intentionally disabled until production R1 deployment".
   - `LeanKohaku/Cli/Commands.lean:776,792,795,832,833` — example commands and
...
| 2 | Add chain param to `r1SendFlow`. Plumb through to the script invocation (`RPC_URL` env, chain-qualified account file). | `LeanKohaku/Daemon/Server.lean:1335` | S |
| 3 | Add chain-aware daemon RPC methods. Either add `r1.sendEth` / `r1.sendRaw` / `tpm.sign` accepting `chain`, or add `*Mainnet` siblings. Recommendation: parameterize, not sibling — sibling explosion is what got us here. | `LeanKohaku/Daemon/Server.lean:1892,1901,1911,1921` | S–M |
| 4 | Make `tpm.listSepoliaAddresses` chain-aware (new RPC `tpm.listAddresses` taking `chain`, or surface both addresses). | `LeanKohaku/Daemon/Server.lean:1876` | S |
...
| 6 | Rename `signSepoliaDigest` → `signDigest`. Update CLI help text & stale "temporary Solidity fallback" comment. | `LeanKohaku/Keystore/Tpm2Runtime.lean:450`, `LeanKohaku/Cli/Commands.lean:776-833`, `script/r1_sepolia.sh:317` | S |
